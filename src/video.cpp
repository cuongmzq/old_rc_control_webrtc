...
typedef std::function<void(void)> fnc;
...
static MMAL_COMPONENT_T *create_camera(MMALCAM_BEHAVIOUR_T *behaviour, MMAL_STATUS_T *status)
{
...
/*****************************************************************************/
static MMAL_COMPONENT_T *create_encoder(MMALCAM_BEHAVIOUR_T *behaviour, MMAL_STATUS_T *status)
{
...
static MMAL_STATUS_T setup_output_port(MMAL_PORT_T *output_port, MMAL_QUEUE_T **p_queue, MMAL_POOL_T **p_pool)
{
   ...
   output_port->userdata = reinterpret_cast<MMAL_PORT_USERDATA_T *>(queue);
...
static MMAL_BOOL_T mmalcam_next_effect(MMAL_COMPONENT_T *camera)
{
   ...
   MMAL_PARAMETER_IMAGEFX_T image_fx = {{ MMAL_PARAMETER_IMAGE_EFFECT, sizeof(image_fx)},static_cast<MMAL_PARAM_IMAGEFX_T>(0)};
   MMAL_PARAMETER_IMAGEFX_T image_fx_check = {{ MMAL_PARAMETER_IMAGE_EFFECT, sizeof(image_fx)},static_cast<MMAL_PARAM_IMAGEFX_T>(0)};
...
static MMAL_BOOL_T mmalcam_next_focus(MMAL_COMPONENT_T *camera)
{
   ...
   static unsigned int index;
   static MMAL_PARAMETER_FOCUS_T focus = {{MMAL_PARAMETER_FOCUS,sizeof(focus)},static_cast<MMAL_PARAM_FOCUS_T>(0)};
   static MMAL_PARAMETER_FOCUS_T focus_check = {{MMAL_PARAMETER_FOCUS,sizeof(focus)},static_cast<MMAL_PARAM_FOCUS_T>(0)};
   MMAL_STATUS_T result;
...
static MMAL_BOOL_T mmalcam_next_drc(MMAL_COMPONENT_T *camera)
{
   ...
   static unsigned int index;
   MMAL_STATUS_T result;
   MMAL_PARAMETER_DRC_T drc = {{MMAL_PARAMETER_DYNAMIC_RANGE_COMPRESSION,sizeof(drc)},static_cast<MMAL_PARAMETER_DRC_STRENGTH_T>(0)};
   MMAL_PARAMETER_DRC_T drc_check = {{MMAL_PARAMETER_DYNAMIC_RANGE_COMPRESSION,sizeof(drc_check)},static_cast<MMAL_PARAMETER_DRC_STRENGTH_T>(0)};
...
int start_recorder(volatile int *stop, MMALCAM_BEHAVIOUR_T *behaviour, std::function<void(MMAL_BUFFER_HEADER_T *)> callback, FrameQueue* f_buffer) {
   ...
   printf("MMAL Camera Test App\n");
    ...
    MMAL_PORT_T *video_port = 0, *still_port = 0;
   ...
   tunneling = behaviour->tunneling;
    ...
    camera = create_camera(behaviour, &status);
    if (!camera) {
    ...
    video_port = camera->output[1];
//    still_port = camera->output[1];

    if (true || behaviour->uri)
    {
      ...
      encoder = create_encoder(behaviour, &status);
      if(!encoder)
      {
      ...
      encoder_input = encoder->input[0];
      encoder_output = encoder->output[0];
        f_buffer->init(encoder_output->buffer_num_recommended * 3, encoder_output->buffer_size_recommended * 3);
